// ============================================================
// FitTraining — Sistema de Prompts para Gemini / Pollinations
...
TIPOS DE SESSÃO DE TREINO HYROX:
  ...
  FORÇA BASE: Força composta para suporte às estações (Squat, Deadlift, Row, Overhead Press).
...
NÍVEL 4 — AJUSTES DE FEEDBACK SEMANAL (weekly_assessment):
  • Se average_rpe > 8: Reduzir volume_modifier em 0.1
  • Se taxa de workouts_completed < 60%: Reduzir volume_modifier em 0.1 (ajustar ao tempo do atleta)
  • Considerar fortemente os "notable_feedback" do atleta para evitar dores reportadas.
...
Devolve EXATAMENTE este JSON:
{
  ...
  "coaching_note": "2-3 frases de programação para passar ao motor de IA",
  "coach_insight": "1-2 frases em tom inspirador dirigidas DIRETAMENTE AO ATLETA, explicando porquê este plano é assim com base na semana anterior. Ex: 'Notei que o treino de pernas foi muito exigente na semana passada e tiveste algumas dores. Por isso, reduzi ligeiramente o volume desta semana para garantir que recuperas bem sem perder rendimento.'"
}
...
REGRAS ABSOLUTAS (aplicam a TODAS as modalidades exceto calistenia):
  ...
  7. CARDIO/CORRIDA: Sempre que houver corrida ou passadeira, deves indicar obrigatoriamente no campo 'reps' ou 'safety_notes' o ritmo pretendido: Velocidade (km/h) e Pace (min/km). Ex: "Ritmo: 12km/h (5:00 min/km)".
  8. safety_notes: SEMPRE mínimo 4 passos numerados de execução técnica. Nunca dicas genéricas.
`.trim(),
...
${modality === 'hyrox' && athleteProfile.athlete.hyrox_division ? "Divisão HYROX: " + athleteProfile.athlete.hyrox_division : ""}
...
REGRAS DE PROGRAMAÇÃO SEMANAL:
  ...
  6. Cardio: sempre distância/calorias/tempo. NUNCA reps numéricas. Sempre que houver corrida ou passadeira, indica a Velocidade (km/h) e o Pace (min/km) alvo.
`.trim(),
...
Equipamento disponível: ${JSON.stringify(planConfig.equipment || [])}
...
Tempo disponível por treino: ${planConfig.session_duration} minutos
${modality === 'hyrox' && planConfig.race_date ? "Data da corrida: " + planConfig.race_date + " | Semanas até à prova: " + planConfig.weeks_to_race : ""}

━━━ DECISÃO DO TREINADOR (APLICA OBRIGATORIAMENTE) ━━━
${planConfig.coachingDecision ? `
Intensity Modifier: ${planConfig.coachingDecision.intensity_modifier ?? 1.0} (multiplica todos os % RM por este valor)
Volume Modifier: ${planConfig.coachingDecision.volume_modifier ?? 1.0} (multiplica todas as séries por este valor, arredonda para inteiro)
Grupos Proibidos esta semana: ${JSON.stringify(planConfig.coachingDecision.forbidden_groups || [])}
Foco Recomendado: ${JSON.stringify(planConfig.coachingDecision.recommended_focus || [])}
Nota do Treinador: "${planConfig.coachingDecision.coaching_note || ''}"
Fase/Sets/Reps: ${JSON.stringify(planConfig.coachingDecision.phase_guidelines || {})}
` : 'Sem decisão do treinador — usa intensidade e volume normais.'}

━━━ HISTÓRICO RECENTE ━━━
${planConfig.history && planConfig.history.length > 0 
  ? planConfig.history.slice(0, 5).map(h => `- ${new Date(h.date).toLocaleDateString('pt-PT')}: ${h.workout} | Score: ${h.score} | Notas: ${h.notes || 'N/A'}`).join('\n')
  : 'Primeiro plano — sem histórico. Começa com volume moderado e progressão conservadora.'}

━━━ REGRAS DE CONCISÃO (CRÍTICO — evita truncamento) ━━━
1. MÁXIMO 4-5 exercícios por dia de treino.
2. 'safety_notes': FORNECE SEMPRE 3 passos técnicos de execução concisos (ex: "1. Pés à largura dos ombros. 2. Dobra a anca. 3. Sobe explosivo."). É ESTRITAMENTE PROIBIDO usar frases genéricas e inúteis como "Garante que realizas o movimento com controlo".
3. 'adaptation': Apenas 1-2 palavras.
4. 'rest_justification': Máximo 1 frase.
5. 'programming_logic': Máximo 1 frase.
6. Se o 'Tempo disponível por treino' for menor que 45 minutos, OMITE a secção 'cooldown' inteira (retorna null ou omite a chave) para focar apenas no principal. Ajusta o volume de treino ('sets' e 'reps') para caber realisticamente no tempo disponível.
  ...
  "days": [
    {
      ...
      "warmup": {
        "description": "string — resumo do aquecimento (ex: 6 min passadeira + mobilidade anca + 2 séries de agachamento)",
        "steps": [
          "string — passo 1 com exercício específico e como o fazer (ex: 'Círculos de tornozelo: 10 reps para cada lado. Faz círculos lentos nos dois sentidos com o pé suspense.')",
          "string — passo 2",
          "string — passo 3 (máx 5 passos)"
        ]
      },
      "exercises": [
        {
      ...
      "cooldown": {
        "description": "string — resumo do retorno à calma",
        "steps": [
          "string — alongamento/método específico e como fazer (ex: 'Pigeon pose: 45s por lado. Deitado, cruza a perna no quadril e puxa suavemente.')",
          "string — passo 2 (máx 4 passos)"
        ]
      }
    },
    {
...
És um coach de CrossFit experiente with foco em programação segura e escalável.
Respondes APENAS em JSON válido. Em Português de Portugal.
